...
def process_word(word, args, input_file, logger):
    ...
    if args.switch_ing:
        total_count += get_inflected_count(word,
                                           infl.inflect_ing, input_file, logger)
    if args.switch_plural:
        total_count += get_inflected_count(word,
                                           infl.inflect_plural, input_file, logger)
    if args.switch_past:
        total_count += get_inflected_count(word,
                                           infl.inflect_past, input_file, logger)
    if args.switch_er:
        total_count += get_inflected_count(word,
                                           infl.inflect_er, input_file, logger)
...
def process_word_in_multiple_files(word, args, input_files_list, logger):
    # This is the default count without inflections
    count = counter.multiple_files_count(word, input_files_list)
    logger.log(" " + word + ":" + str(count))
    ...
    if args.switch_ing:
        total_count += get_inflected_count(word,
                                           infl.inflect_ing, input_files_list, logger)
    if args.switch_plural:
        total_count += get_inflected_count(word,
                                           infl.inflect_plural, input_files_list, logger)
    if args.switch_past:
        total_count += get_inflected_count(word,
                                           infl.inflect_past, input_files_list, logger)
    if args.switch_er:
        total_count += get_inflected_count(word,
                                           infl.inflect_er, input_files_list, logger)
