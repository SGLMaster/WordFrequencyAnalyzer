...
def process_word(word, args, filenames):
    ...
    if args.switch_ing:
        word_counter.set_ing_count(get_inflected_count(
            word, infl.inflect_ing, filenames))
    if args.switch_plural:
        word_counter.set_plural_count(get_inflected_count(
            word, infl.inflect_plural, filenames))
    if args.switch_past:
        word_counter.set_past_count(get_inflected_count(
            word, infl.inflect_past, filenames))
    if args.switch_er:
        word_counter.set_er_count(get_inflected_count(
            word, infl.inflect_er, filenames))
